...
def _import_dhg():
    """Lazily import `dhg` and its nn submodule.

    Delays optional dependencies (e.g., `optuna` pulled in by some dhg versions)
    until an HGNN model is actually constructed.
    """
    try:
        ...
        raise ImportError(
            "HGNN requires the `dhg` package. Install via `pip install dhg` "
            "(note: some versions also require `optuna`)."
        ) from exc


class HGNN(nn.Module):
    r"""The HGNN backbone defined in https://arxiv.org/pdf/1809.09401.pdf.

    Args:
        in_channels (int): Dimension of input node features.
        hid_channels (int): Dimension of hidden representation.
        num_classes (int): Number of output classes.
        num_layers (int, optional): Number of HGNNConv layers. Default: 2.
    ...
    def __init__(
        ...
        super().__init__()
        # Lazy import to prevent importing optional deps at module import time
        _, dhg_nn = _import_dhg()
        HGNNConv = getattr(dhg_nn, "HGNNConv")
...
class HyperGraphHGNN(nn.Module):
    """Wrapper providing the same interface as HyperGraphGCN but using HGNN as backbone."""
